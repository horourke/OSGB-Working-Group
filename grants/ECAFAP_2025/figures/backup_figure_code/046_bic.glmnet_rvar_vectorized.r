...
if (example) {
  ...
  #########################
  {
    ...
    par(mfrow = c(2,1))
    mat <- sims_data$B_dcmp$phi0
    edges <- which(mat != 0, arr.ind = TRUE)
    g <- graph_from_data_frame(edges, directed = T, vertices = 1:5)
    ...
    par(mfrow = c(2,1))
    mat <- sims_data$B_dcmp$phip_list[[1]]
    edges <- which(mat != 0, arr.ind = TRUE)
    edges <- rbind(edges, cbind(1:5, 1:5))
    g <- graph_from_data_frame(edges, directed = T, vertices = 1:5)
    plot(mat, main = "Joint Effect", breaks = col_lims)
    plot(g,
         edge.width = E(g)$weight * 8, # Edge width based on weight
    ...
    par(mfrow = c(2,1))
    mat <- sims_data$B_dcmp$phip_list[[2]]
    edges <- which(mat != 0, arr.ind = TRUE)
    edges <- rbind(edges, cbind(1:5, 1:5))
    g <- graph_from_data_frame(edges, directed = T, vertices = 1:5)
    plot(mat, main = "Joint Effect", breaks = col_lims)
