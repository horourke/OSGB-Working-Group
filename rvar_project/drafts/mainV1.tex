...
Unfortunately, we are not really able to use the \texttt{glmnet} package for implementing our solution to the R-VAR problem. This is in part because the multi-task regression option provided by the \texttt{glmnet} package defaults to applying a row-wise group LASSO penalty, instead of the desired entry-wise penalty. I explored the possibility of using a different package for fitting the multi-task learning problem, such as the  \texttt{RMTL} package. Despite my efforts, I realized that the package would not help since it does not allow for weighted problems. This leaves me in the realization that we need to optimize the problem with our own functions. I think this is doable, but will require effort.
