...
solve_rvar_glmnet_vectorized <- function(Y_list, X, p, 
  ...
  for (pf_val in penalty.factor) {
    ...
    glmnet_sparse <- glmnet(
      ...
      lambda  = lambda.seq,
      penalty.factor = pf_vec, intercept = FALSE)
...
if (example) {
  ...
  lambda.seq      <- 10^(seq(1, -3, length.out = 20))
  penalty.factor  <- 10^(seq(3, -3, length.out = 20))
  verbose <- TRUE
  
  glmnet_model <- solve_rvar_glmnet_vectorized(
  ...
  par(mfrow = c(3,2))
  for (i in 5:10) {
    glmnet_model[100 * (i - 1) + (1:100),] %>% select(-lambda1, -lambda2, -var) %>%
    as.matrix() %>%
    plot()
  } ## Some examples... but to be honest it is quite hard to visualize... 
