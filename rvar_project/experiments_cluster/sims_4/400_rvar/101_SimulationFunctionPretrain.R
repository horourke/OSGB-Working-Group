  ...
  while (sim_ind < args$nsim + 1) {
    ...
    ######## Sparse RVAR: adaptive
    {
      ...
      output[count, 4]      <- difftime(
          ...
          as.numeric()
      output[count, -(1:4)] <- eval_msr(data$B_list, bic.model$var_ind_coeffs, Y_forecast, args$hrange)
    ...
    ######## BIC adaptive
    {
      ...
      output[count, -(1:4)] <- eval_msr(data$B_list, bic.model_ad$var_ind_coeffs, Y_forecast, args$hrange)
      ...
      gather$BICmodel_ad <- bic.model_ad
    ...
    ######## CV-RVAR
    {
      ...
      output[count, 4]      <- difftime(
          time1 = end_time, time2 = start_time, units = "s") %>%
          as.numeric()
      output[count, -(1:4)] <- eval_msr(data$B_list, cv.model$var_ind_coeffs, Y_forecast, args$hrange)
    ...
    ######## CV adaptive
    {
      ...
      output[count, 4]      <- difftime(
          time1 = end_time, time2 = start_time, units = "s") %>%
          {as.numeric(.) + output[count - 1, 4]}
      output[count, -(1:4)] <- eval_msr(data$B_list, cv.model_ad$var_ind_coeffs, Y_forecast, args$hrange)
