...
df <- read.csv("BrainNetworksGeneData/reduced_network_signals_data.csv")
...
##############################################
## Lets do the same exploration with SN_GLobal:

## By-subject condition number.
...
df %>%
  ...
  cov() %>% eigen() %>% 
  {max(.$values)/min(.$values)}
## I think we should still remove it, since there is ambiguity of interpretation.
...
##############################################
## Final analysis of condition number
df_list %>% lapply( function(x) {
  x %>%
    dplyr::select(-sub, -t) %>% 
    cov() %>% eigen() %>% 
...
save.image("BrainNetworksGeneData/100_Data.RData")
