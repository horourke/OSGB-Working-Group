...
##                      penalty for the moderators and multi-var effects.
##      weights     : optional weight matrix (q x d) for weighted lasso.
##      var_fits    : optional list of subject-level VAR model fits used to
##                      calculate adaptive weights when weights = NULL.
##      modvar_fit  : optional MOD-VAR fit object used to calculate adaptive
##                      weights when weights = NULL.
...
cv.modvar <- function(
    ...
    ## Construct weights
    if (is.null(weights)) {
        if (p == 0 && multi && !is.null(var_fits) && is.null(modvar_fit)) {
            weights <- adaweights(
                modvar_fit = NULL,
                var_fits = var_fits,
                p = 0,
    ...
    L0norm <- array(0, c(nlam1, nrat, length(cv.windows[[1]])))
    ...
    for (ind in seq_along(cv.windows[[1]])) {
        ...
        ## Number of nonzero coefficients for each tuning parameter
        for (i in seq_len(nlam1)) {
            for (j in seq_len(nrat)) {

                ind.flat <- (j - 1) * nlam1 + i

                Bij <- matrix(Bout[, , ind.flat], nrow = q, ncol = d)

                L0norm[i, j, ind] <- sum(abs(Bij) > 1e-8)
            }
        }
    ...
    CV.SD <- apply(MSFE, c(1,2), sd)
    l0norm_mat <- apply(L0norm, c(1,2), mean)
    ...
    if (!sparse1sd) {
        ...
        lambda1_opt <- lambdas1[opt.i]
        ratio_opt   <- ratios[opt.j]

    } else {

        ## One-SD threshold
        threshold <- CV.MSFE[opt.i, opt.j] + CV.SD[opt.i, opt.j]

        eligible <- CV.MSFE <= threshold

        sparse.score <- l0norm_mat
        sparse.score[!eligible] <- Inf

        sparse.inds <- which(
            sparse.score == min(sparse.score),
            arr.ind = TRUE
        )

        ## Break ties using CV error
        if (nrow(sparse.inds) > 1) {
            ...
            best <- which.min(cvvals)

            opt.i <- sparse.inds[best,1]
            opt.j <- sparse.inds[best,2]

        } else {

            opt.i <- sparse.inds[1,1]
            opt.j <- sparse.inds[1,2]
        }
