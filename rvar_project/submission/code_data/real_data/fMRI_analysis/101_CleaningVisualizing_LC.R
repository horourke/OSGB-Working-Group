...
df <- read.csv("reduced_network_signals_data.csv")
lcdf <- read_xlsx("LC_CNRs_for_Fox_Controls.xlsx")
...
##########################################
##########################################
## Visualizing the time-series data for the first
## 5 subjects.
library(dplyr)
library(tidyr)
library(ggplot2)
...
##########################################
##########################################
## Including packages for testing presence
...
##############################################
##############################################
## Visualization of lag-1 autocorrelation:
par(mfrow = c(3,3))
df_list[1:9] %>%
  lapply(function(x) {
    n <- nrow(x) -1
...
## 1) We visualize the eigenvalues of the correlation
...
## We calculate the minimum eigenvalue of the 
...
df_list %>%  
  ...
  unlist() 
## It seems like most covariances are rank deficient. 
par(mfrow = c(1,1))
vectors <- df_list %>%  
...
## Yes: there is collinearity because of the global variables.
...
## Since global variables are aggregates of the
## other variables, we explore other global variables 
## to see if they are also collinear with the other variables. 
...
## Collinearity confirmed. Removing SN_Global from the data frame.
...
## Collinearity confirmed. Removing FPN_Global from the data frame.
...
##############################################
##############################################
## Visualizing Locus Coeruleus data.
